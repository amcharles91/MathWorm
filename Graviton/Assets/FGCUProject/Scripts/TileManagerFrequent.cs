...
public class TileManagerFrequent : MonoBehaviour {
    ...
    private void addExtraTile()
    {
        ...
        if (score.GetComponent<ManageScore>().score >= 10)
        {
            
            if(addTilecounter == 0)
            {
                tiles.Add(Resources.Load("sideWaytile", typeof(GameObject)) as GameObject);
                addTilecounter++;
            }
    ...
    private int RandomFab()
    {
        ...
        if (tileCount < 3 && lastIndex < 4)
        {
            while (randomNum == lastIndex)
            {
                randomNum = UnityEngine.Random.Range(3, tiles.Count);
            }

            tileCount++;
            Debug.Log("tileCount increased");
        } else
        {
            while (randomNum == lastIndex)
            {
